...
describe("_lib", function()
    ...
    describe("now_iso()", function()
        ...
        it("returns an RFC3339-ish UTC string with ms", function()
            local s = lib.now_iso()
            assert.is_truthy(s:match("^%d%d%d%d%-%d%d%-%d%dT%d%d:%d%d:%d%d%.%d%d%dZ$"),
                "got " .. s)
        end)
    end)
    ...
    describe("normalize_path()", function()
        ...
        it("respects home_override when supplied", function()
            assert.are.equal("/other/.claude",
                lib.normalize_path("~/.claude", "/other"))
        end)

        it("collapses dot-segments + dedupes slashes", function()
            assert.are.equal("/a/b/c",
                lib.normalize_path("/a//b/./c"))
            assert.are.equal("/etc/foo",
                lib.normalize_path("/etc/bar/../foo"))
        end)
    end)
    ...
    describe("match_paths()", function()
        ...
        it("resists dot-segment escape", function()
            assert.is_true(lib.match_paths(
                "/home/test/.claude/../.claude/CLAUDE.md", exact, prefixes))
        end)
    ...
    describe("append_jsonl()", function()
        ...
        it("creates dir + appends one line per call", function()
            ...
            local f = io.open(d .. "/tool-calls.jsonl"); assert.is_not_nil(f)
            local lines = {}
