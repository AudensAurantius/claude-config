...
describe("git-guard", function()
    ...
    describe("tokenize()", function()
        ...
        it("preserves single-quoted strings literally", function()
            assert.are.same({"git", "log", "--grep=foo bar"},
                gg.tokenize("git log '--grep=foo bar'"))
        end)
    ...
    describe("find_git_verb_in_command()", function()
        ...
        it("skips git-level flag/value pairs", function()
            assert.are.equal("status",
                gg.find_git_verb_in_command({"git", "-C", "/repo", "status"}))
            assert.are.equal("status",
                gg.find_git_verb_in_command({"git", "-c", "user.name=x", "status"}))
            assert.are.equal("commit",
                gg.find_git_verb_in_command({"git", "--git-dir", "/r/.git", "commit"}))
        end)

        it("handles joined --flag=value forms", function()
            assert.are.equal("status",
                gg.find_git_verb_in_command({"git", "--git-dir=/r/.git", "status"}))
        end)
    ...
    describe("classify_verb()", function()
        it("recognizes safe read-only verbs", function()
            for _, v in ipairs({"log", "diff", "show", "status", "ls-files",
                                "rev-parse", "cat-file", "blame"}) do
                assert.are.equal("safe", gg.classify_verb(v, {}),
                    "expected safe: " .. v)
            end
        end)
        ...
        it("gates the obviously-dangerous verbs", function()
            for _, v in ipairs({"push", "commit", "merge", "rebase", "reset",
                                "checkout", "switch", "clean", "stash"}) do
                assert.are.equal("gated", gg.classify_verb(v, {}),
                    "expected gated: " .. v)
            end
        end)
    end)
